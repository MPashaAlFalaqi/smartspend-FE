...
export default function BudgetPlanner() {
  ...
  // State nama user yang sinkron dengan localStorage
  const [namaUser, setNamaUser] = useState(
    localStorage.getItem('namaUser') || localStorage.getItem('user_name') || 'User'
  )

  // SINKRONISASI AVATAR: Mengambil data avatar user dari localStorage
  ...
  // Efek samping tambahan untuk memastikan data lokal selalu segar saat komponen di-mount
  useEffect(() => {
    const localAvatar = localStorage.getItem('user_avatar')
    const localName = localStorage.getItem('namaUser') || localStorage.getItem('user_name')
    if (localAvatar) setAvatarUser(localAvatar)
  ...
  const handleJumlah = (tab, id, val) => {
    const rawValue = val.replace(/[^0-9]/g, '') // Hapus karakter selain angka
